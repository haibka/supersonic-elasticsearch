	...
	public static void sendMessageCustomerPay(Statement stmt, Integer oldVersionCustomerPay) throws SQLException, IOException {
	    if (!checkVersion(stmt).equals(oldVersionCustomerPay)) {
	        ...
	        String SQL = "DECLARE @PreviousVersion bigint\n" + 
	        		...
	        		"Cp.AZUKARI_STATUS_ID, Cp.SHOP_CODE, Cp.NP_TRANSACTION_ID, Cp.RECEIPTS_AMOUNT\n" + 
	        		"FROM CHANGETABLE (CHANGES TBLCUSTOMER_PAY, @PreviousVersion) AS CTTable\n" + 
	        		"LEFT OUTER JOIN TBLCUSTOMER_PAY AS Cp\n" + 
	        		"ON Cp.ID = CTTable.ID" ;
		    ResultSet rs = stmt.executeQuery(SQL);
		    String status;
		    Integer checkChange = 0;
		    String fileName = new SimpleDateFormat("yyyyMMdd_HHmmss").format(Calendar.getInstance().getTime());
		    PrintWriter writer = null;
		    PrintWriter writerVersion = null;
		           
		    while (rs.next()) {
		    	if (checkChange == 0) { // chi khoi tao file ghi log khi co su thay doi
		    		checkChange++;
		    		writer = new PrintWriter("./change_customer_pay/" + fileName + ".csv", "UTF-8");
		    		writerVersion = new PrintWriter("./change_customer_pay/version.txt", "UTF-8");
		    	}
				...
				if(!status.equals("D")) {
					...
					writer.append(COMMA_DELIMITER);
					writer.append(rs.getString("SHOP_CODE"));
					writer.append(COMMA_DELIMITER);
					writer.append(rs.getString("NP_TRANSACTION_ID"));
					writer.append(COMMA_DELIMITER);
					writer.append(rs.getString("IR_FLG"));
					writer.append(COMMA_DELIMITER);
					writer.append(rs.getString("TRANSFER_NAME"));
					writer.append(COMMA_DELIMITER);
		    ...
		    if(checkChange != 0) {
			    ...
			    writer.close();
			    String keyName = "data_update/customer_pay/" + fileName + ".csv";
		        String filePath = "./change_customer_pay/" + fileName + ".csv";
			    LowLevelMultipartUpload.sendFile(keyName, filePath);
			    writerVersion.close();
	...
	public static void sendMessageAzukariHistory(Statement stmt, Integer oldVersionAzukariHistory) throws SQLException, IOException {
	    if (!checkVersion(stmt).equals(oldVersionAzukariHistory)) {
		    ...
		    while (rs.next()) {
		    	if (checkChange == 0) {
		    		checkChange++;
		    		writer = new PrintWriter("./change_azukari_history/" + fileName + ".csv", "UTF-8"); 
		    		writerVersion = new PrintWriter("./change_azukari_history/version.txt", "UTF-8");
		    	}
				status = rs.getString("SYS_CHANGE_OPERATION");
				writer.append(status);
			    writer.append(COMMA_DELIMITER);
				...
				if(!status.equals("D")) {
					...
					writer.append(rs.getString("AZUKARI_STATUS_ID"));
					writer.append(COMMA_DELIMITER);
					writer.append(StringSplit(rs.getString("MEMO")));
					writer.append(COMMA_DELIMITER);
		    ...
		    if( checkChange != 0) {
		    	writerVersion.println(Integer.toString(checkVersion(stmt)));
			    writer.close();
			    String keyName = "data_update/azukari_history/" + fileName + ".csv";
		        String filePath = "./change_azukari_history/" + fileName + ".csv";
			    LowLevelMultipartUpload.sendFile(keyName, filePath);
			    writerVersion.close();
	...
	public static void sendMessageSeikyu(Statement stmt, Integer oldVersionSeikyu) throws SQLException, IOException {
	    if (!checkVersion(stmt).equals(oldVersionSeikyu)) {
		    ...
		    while (rs.next()) {
		    	if (checkChange == 0) {
		    		checkChange++;
		    		writer = new PrintWriter("./change_seikyu/" + fileName + ".csv", "UTF-8");
		    		writerVersion = new PrintWriter("./change_seikyu/version.txt", "UTF-8");
		    	}
				status = rs.getString("SYS_CHANGE_OPERATION");
				writer.append(status);
			    writer.append(COMMA_DELIMITER);
		    ...
		    if (checkChange != 0) {
			    writerVersion.println(Integer.toString(checkVersion(stmt)));
			    writer.close();
			    String keyName = "data_update/seikyu/" + fileName + ".csv";
		        String filePath = "./change_seikyu/" + fileName + ".csv";
			    LowLevelMultipartUpload.sendFile(keyName, filePath);
			    writerVersion.close();
	...
	public static void sendMessageNyukin(Statement stmt, Integer oldVersionNyukin) throws SQLException, IOException {
	    ...
	    if (!checkVersion(stmt).equals(oldVersionNyukin)) {
		    ...
		    while (rs.next()) {
		    	if (checkChange == 0) {
		    		checkChange++;
		    		writer = new PrintWriter("./change_nyukin/" + fileName + ".csv", "UTF-8");
		    		writerVersion = new PrintWriter("./change_nyukin/version.txt", "UTF-8");
		    	}
				status = rs.getString("SYS_CHANGE_OPERATION");
				writer.append(status);
			    writer.append(COMMA_DELIMITER);
		    ...
		    if(checkChange != 0) {
			    ...
			    writer.close();
			    String keyName = "data_update/nyukin/" + fileName + ".csv";
		        String filePath = "./change_nyukin/" + fileName + ".csv";
			    LowLevelMultipartUpload.sendFile(keyName, filePath);
			    writerVersion.close();
		    }
		  ...
		  public void run() {
			// Create a variable for the connection string.
		        try (Connection con = DriverManager.getConnection(connectionUrl); Statement stmt = con.createStatement();) {
		            ...
		            sendMessageCustomerPay(stmt, getVersion("./change_customer_pay/version.txt"));
		            sendMessageAzukariHistory(stmt, getVersion("./change_azukari_history/version.txt"));
		            sendMessageSeikyu(stmt, getVersion("./change_seikyu/version.txt"));
		            sendMessageNyukin(stmt, getVersion("./change_nyukin/version.txt"));
		            con.close();
		            stmt.close();
		        }
